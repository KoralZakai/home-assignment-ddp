...
def main():
    ...
    # 1. Define the GPT-2 Large (~774M) architecture configuration (no weights downloaded).
    # Values from the GPT-2 paper, Table 2, "762M" row. d_head = n_embd / n_head = 64.
    config = GPT2Config(
        vocab_size=50257,        # Standard GPT-2 vocabulary size
        n_positions=1024,        # GPT-2 standard context window length
        n_embd=1280,             # Hidden dimension size (GPT-2 Large)
